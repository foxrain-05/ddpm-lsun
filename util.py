...
def download1(url, filename, chunk_size= 1024*1024*500):
    retry_count = 0
    while retry_count < 5:
        ...
        try:
            ...
            progress_bar.close()
            print(f'download complete. 다운로드 완료. {filename}')
            return
    ...
    raise UserWarning(f"Download failed...  다운로드에 실패했습니다... {filename}")
